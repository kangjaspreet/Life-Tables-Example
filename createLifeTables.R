...
# Script dependencies:
# R packages (some of these packages may not be needed anymore):
  # - pacman
  # - readr
  # - readstata13
  # - stringr
...
# Inputs:
# countyLink.xlsx - county name to county FIPS linkage file
# Tract to Community Linkage.csv - census tract IDs to community (MSSA) IDs
# racelink.xlsx - CHSI race codes (in raw death data) to CCB race codes/names
# nxCounty.RDS - county-level population data created from 'createPopulationData.R' - Contains every year(2000-)-county-sex(including total)-race(including total)-ageGroup combination
# nxState.RDS - state-level population data created from 'createPopulationData.R' - Contains every year(2000-)-state-sex(including total)-race(including total)-ageGroup combination
# nxMSSA.RDS - MSSA-level population data created from 'createPopulationData.R' - Contains every year(2007-)-MSSA-sex-race(including total)-ageGroup combination
# ccb_processed_deaths_FAKE.RDS - semi-processed, record-level death data created further upstream; Contains FAKE death data from 2000-. This is a fake data file to protect sensitive and personal information.
...
# Script structure:
# 1. Setup - Load required R packages, set global constants, source standard and geography linkage files, define function
...
controlPop  <-  F     # whether to control ACS to DOF pop totals... NOTE: MSSA population data used in this script are NOT controlled to DOF pop totals
critNx      <- 10000 # Population threshold - Only calculate life tables for strata at or above this threshold
critDx      <-   700 # Number of deaths threshold - Only calculate life tables for strata at or above this threshold
...
nxCounty <- readRDS("dataIn/nxCounty.RDS") # Contains every year(2000-)-county-sex(including total)-race(including total)-ageGroup combination
nxState <-  readRDS("dataIn/nxState.RDS") # Contains every year(2000-)-state-sex(including total)-race(including total)-ageGroup combination
...
# Real death data
dx <- readRDS("dataIn/ccb_processed_deaths_FAKE.RDS") %>%
    filter(year <= myYear) %>%
...
dxCounty <- dx %>%
  ...
  mutate(agell = ageChop(INAGE = age, # Using age chop function to create age lower limits
...
saveRDS(ltState, "dataOut/LTciState.RDS")
saveRDS(ltCounty, "dataOut/LTciCounty.RDS")
saveRDS(ltMSSA, "dataOut/LTciMSSA.RDS")
...
saveRDS(ltCounty_atBirth, "dataOut/e0ciCounty.RDS")
saveRDS(ltMSSA_atBirth, "dataOut/e0ciMSSA.RDS")
...
# Note: Some of these notes may be outdated now due to enhancements made to the original process and code by the CCB team.
